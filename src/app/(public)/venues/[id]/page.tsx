...
import { VenueMapStatic } from "@/components/map/venue-map-static";
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-24 space-y-4">
                ...
                />
                {/* Contact owner button — only show if logged in and not the owner */}
                {session?.user?.id && session.user.id !== (venue.owner as any)?.id && (
                  <div className="bg-card rounded-2xl border border-border p-4">
                    <p className="text-sm font-medium mb-1">¿Tienes alguna pregunta?</p>
                    <p className="text-xs text-muted-foreground mb-3">
                      Contacta directamente con el propietario antes de reservar.
                    </p>
                    <MessageButton
                      recipientId={(venue.owner as any).id}
                      venueId={venue.id as string}
                      venueName={venue.title as string}
